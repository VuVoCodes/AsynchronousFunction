...
3. **New control run testing your mechanism directly.** If the attenuation is an optimizer property rather than a method or data property, SGD on the identical MOSI pipeline should raise transmission. The results are consistent with this: counting the fraction of the applied excess scale that reaches parameter updates, the modality boosted hardest under Adam transmits 35% under Adam ((1.17-1)/(1.48-1)) and 62% under SGD ((1.18-1)/(1.29-1)), and the most-boosted SGD modality transmits fully (1.47 against 1.45). The controller boosts different modalities under the two optimizers, so each row reports that run's most-boosted modality, and the control is a short instrumentation run at matched steps and seed. We will provide the full per-modality table for both optimizers.
