...
% Not loaded: math_commands.tex redefines \eqref as "equation~\ref{...}",
% which breaks the "Eq.~\eqref{...}" references used throughout this paper.
...
% ("Anonymous authors") as long as \iclrfinalcopy remains commented out below.
\author{%
  Vo Quoc Vu \\
...
Probing is performed by attaching a lightweight linear classifier to each modality’s stop-gradient features to estimate the corresponding representation quality score. The unbiased gap between these scores across modalities, the \emph{utilization gap}, serves as an imbalance signal that defines a bounded and smoothed scaling factor adaptively reweighing the gradients of weaker modalities, enabling stable and effective rebalancing during joint optimization.
...
Multimodal learning integrates complementary signals across heterogeneous sources, including audio, visual, and textual streams among others~\citep{baltrusaitis2019multimodal}. Under mild assumptions on latent structure, multimodal models are provably superior to their unimodal counterparts~\citep{huang2021multimodal}. Despite this theoretical promise, jointly trained multimodal networks frequently fail to fully exploit all available modalities, often performing no better than their strongest unimodal component. This \emph{modality laziness} effect~\citep{wang2020gblending,peng2022ogmge,du2023suppression} has been observed for instance in audio-dominant emotion recognition on the CREMA-D~\citep{cao2014cremad} dataset.
...
\begin{itemize}
    ...
    \item We propose \emph{decoupled probe monitoring}, a diagnostic attaching lightweight linear probes to stop-gradient features, yielding a unbiased \emph{utilization gap} estimate (the spread of per-modality probe accuracies) independent of the joint objective. The decoupling enables effective weak-modality boosting where prior coupled-monitoring boost-the-weak attempts failed~\citep{wei2024opm}.

    \item We introduce \emph{PGGB}, one instantiation that converts the monitoring signal into a bounded multiplicative scaling on the weaker encoder's gradients. It is composable with existing throttling methods (Appendix~\ref{app:composability} validates non-destructive composition with four such methods).

    \item We validate \emph{PGGB} on eight benchmarks spanning audio-visual, sentiment, text-image, and medical-segmentation tasks. \emph{PGGB} composed with throttling outperforms state-of-the-art baselines on high-imbalance audio-visual (CREMA-D), matches the strongest available baseline on medical segmentation (BraTS~2021, $+0.28$~pp, within seed variance), and remains competitive within seed variance on the high-imbalance text-dominant sentiment benchmarks (CMU-MOSI, CMU-MOSEI), and \emph{PGGB} used alone is the best-performing method on all four low-imbalance datasets (AVE, Kinetics-Sounds, Twitter15, Sarcasm).
\end{itemize}
...
The dominant response has been to modulate gradient magnitudes during backpropagation. Multi-task precursors GradNorm~\citep{chen2018gradnorm} and PCGrad~\citep{yu2020pcgrad} inspired a line of multimodal work. OGM-GE~\citep{peng2022ogmge} dampens the dominant modality's gradient via a ratio of per-modality contributions to the target-class score, refined by AGM~\citep{li2023agm}, CGGM~\citep{guo2024cggm}, and MLGM~\citep{kontras2024mlgm}. Although some methods include bidirectional terms, throttling empirically dominates and boost-only variants remain ineffective: the symmetric boost direction (OGM*) reported in~\citet{wei2024opm} fails to close the modality gap, which we attribute to coupled monitoring signals affected by the dominant modality (Section~\ref{sec:method}). M-SAM~\citep{nowdeh2025msam} pairs Shapley attribution with sharpness-aware perturbation, inheriting the same coupled-monitoring limitation.
...
We present PGGB in four stages: problem formulation (\S\ref{sec:problem}), decoupled probe monitoring (\S\ref{sec:probes}), the boost mechanism (\S\ref{sec:boost}), and composability with existing balancing methods (\S\ref{sec:compose}). Figure~\ref{fig:architecture} traces the data flow: the top panel depicts the forward path (\S\ref{sec:problem}), the bottom panel depicts probe monitoring on stop-gradient features (\S\ref{sec:probes}), and the green boost block plus actuation rail depict the gradient-modification mechanism (\S\ref{sec:boost}). Composition with throttling is described in (\S\ref{sec:compose}).
...
\end{equation}
%
Each $\mathbf{z}_m$ is a real value feature vector in $\mathbb{R}^d$, with $d$ common across modalities. 
%
\begin{equation}
\label{eq:joint_loss}
\loss_{\text{fusion}} = \ell\!\bigl(\hat{y},\; y\bigr).
\end{equation}
%
Here $\ell$ is a differentiable per-sample loss mapping a prediction-target pair to a non-negative scalar. By the chain rule, the gradient with respect to encoder parameters decomposes as
%
\begin{equation}
...
\caption{An Illustration of probe-guided gradient boosting on 2 modalities. \textbf{Top panel:} features $\mathbf{z}_m$ are concatenated and classified via the shared fusion classifier $g$, producing $\loss$. \textbf{Bottom panel:} stop-gradient features $\sg[\mathbf{z}_m]$ are evaluated by linear probes along solid per-modality probe-flow lines (audio in blue, visual in orange), yielding per-modality accuracies $\bar{P}_m$. The boost block computes the relative-gap weight $w_m$ and a smoothed scale $\bar{s}_m$ (EMA-smoothed, refreshed only at probe-evaluation events every $K$ steps and held constant in between), clipped at $s_{\max}$. The actuation rail (thick green) applies $\bar{s}_m$ to the per-modality gradient $\nabla_{\!\theta_m}\loss$. Probes never backpropagate into encoders, so the monitoring signal is structurally decoupled from the fusion objective.}
...
We attach a lightweight linear probe classifier $\probe_m\colon \mathbb{R}^d \to \mathbb{R}^C$ (with $C$ outputs) to each encoder, parameterized by $\psi_m$. The resulting probe accuracies define an imbalance signal, the \emph{utilization gap} (Eq.~\ref{eq:utilization_gap}), independent of the joint training objective. The probes are trained on stop-gradient features $\sg[\mathbf{z}_m]$, where $\sg[\cdot]$ denotes the stop-gradient operator~\citep{vandenoord2017vqvae} (identity at the forward pass, zero partial derivatives at the backward pass):
%
\begin{equation}
\label{eq:probe_loss}
\loss_{\text{probe},m} = \ell_{\text{probe}}\!\bigl(\probe_m(\sg[\mathbf{z}_m];\,\psi_m),\; y\bigr).
\end{equation}
%
Each probe has its own optimizer that updates only $\psi_m$, and no gradient flows into $\theta_m$. Since $\sg[\mathbf{z}_m]$ halts backward propagation at the feature level, $\loss_{\text{probe},m}$ depends only on the quality of $\mathbf{z}_m$ as a representation, not on how it contributes through $g$. This is precisely why decoupling matters: the probe accuracy $P_m$ measures representation quality not influenced by the dominant modality as the influence can be both ways.

\paragraph{Split-batch evaluation and on-the-fly estimation.}
Every $K$ training iteration, we partition the (shuffled) mini-batch into two disjoint halves, training probes on the first half and evaluating on the second. This yields a unbiased estimate $P_m$ of the current probe's training-distribution accuracy (conditioned on $\psi_m^{(t)}$), the evaluation half is held out from probe training and drawn i.i.d.\ from the training distribution. To convert this per-evaluation sample into a stable estimator, we accumulate an exponential moving average (EMA):
%
\begin{equation}
\label{eq:ema_probe}
...
Given the EMA-smoothed probe accuracies $\{\bar{P}_m\}_{m=1}^{M}$ (Eq.~\ref{eq:ema_probe}), we compute a per-modality boost scale that amplifies the gradients of weaker encoders (full pseudocode in Appendix~\ref{app:algorithm}, Algorithm~\ref{alg:pgb}). The procedure involves three steps.
...
\end{equation}
%
where $j$ ranges over all modalities $\{1, \ldots, M\}$, and $\epsilon$ is a small positive constant serving as a numerical guard against division by zero.

\paragraph{Step 2: Boost scale.}
$w_m$ is converted to a multiplicative scaling factor:
%
\begin{equation}
...
\end{equation}
%
where $\mu \in (0,1)$ is the EMA smoothing coefficient on the boost scales (set in Section~\ref{sec:experiments}). The smoothed scales are applied to the encoder gradients \emph{after} the backward pass and \emph{before} the optimizer step:
%
\begin{equation}
...
\end{equation}
%
where $\omega_m^{\text{ogm}} \leq 1$ is the OGM-GE throttle and $\bar{s}_m \geq 1$ is our boost scale. 
...
\begin{equation}
\label{eq:total_loss}
\loss = \loss_{\text{fusion}} + \gamma \sum_{m=1}^{M} \ell_{\text{aux}}\!\bigl(g_m(\mathbf{z}_m),\; y\bigr),
\end{equation}
%
where $\gamma$ is a scalar weight on the per-modality auxiliary terms (set in Section~\ref{sec:experiments}). $\ell_{\text{aux}}$ matches the form of $\ell$ (Eq.~\ref{eq:joint_loss}) for each benchmark.
...
We establish three safety properties: bounded scaling, self-attenuation, and a standard-SGD descent bound. These are structural guarantees that the method does not diverge or harm low-imbalance training, not mechanism-specific convergence results. A rate-level analysis tying gap closure to probe dynamics is left for future work. Full proofs are in Appendix~\ref{app:proofs}.
...
The bound matches the standard nonconvex SGD rate~\citep{ghadimi2013sgd} up to an $s_{\max}^2$ factor in the variance term, a controlled increase ($s_{\max}=2$), and Proposition~\ref{prop:attenuation} ensures this cost is incurred only when imbalance is present. The smoothness constant $L$ in Assumption~\ref{asm:smooth} is loss-dependent, determined by the choice of $\ell$ in Eq.~\eqref{eq:joint_loss}; in our experiments, the per-task learning rate (Section~\ref{sec:experiments}) is selected consistent with the descent inequality for the corresponding $\ell$.
...
We categorize benchmarks by the utilization gap $\delta$ (Eq.~\eqref{eq:utilization_gap}) measured during baseline training, with per-dataset values in Appendix~\ref{app:delta}: \emph{high-imbalance} ($\delta > 0.15$, CREMA-D, CMU-MOSI, CMU-MOSEI) and \emph{low-imbalance} ($\delta \leq 0.15$, AVE, KS, Twitter15, Sarcasm). BraTS~2021 is reported separately as a dense-prediction segmentation benchmark. The high-imbalance group is heterogeneous in source: CREMA-D imbalance arises from audio-dominance in emotion recognition~\citep{peng2022ogmge}, whereas CMU-MOSI/MOSEI imbalance reflects text-sufficiency, with text-only baselines within roughly $1$~pp of multimodal accuracy on CMU-MOSEI~\citep{li2023agm} and only a $+2.7$~pp multimodal gain on CMU-MOSI~\citep{wei2025dgl}. On these text-dominant benchmarks, balancing methods report only modest absolute gains~\citep{guo2024cggm,wei2024opm}, motivating within-seed-std competitiveness as the relevant metric.
...
Table~\ref{tab:main_results} reports results across eight benchmarks. On high-imbalance CREMA-D, PGGB+OGM-GE attains $71.45\%$, $+2.31$~pp over OGM-GE and $+9.86$~pp over baseline, the best among competitors. A three-way decomposition in \S\ref{sec:ablations} isolates this gain to the boost actuation, with the decoupled monitor causally inert at $\alpha{=}0$. PGGB alone ($+1.13$~pp) is insufficient under severe imbalance, confirming that two-sided intervention is required. On the four low-imbalance benchmarks (AVE, KS, Twitter15, Sarcasm, with cross-class MSLR comparison in Appendix~\ref{app:mslr}), PGGB alone is the best-performing method on all four, whereas OGM-GE often regresses (KS $-1.80$~pp, Twitter15 $-0.27$~pp, Sarcasm $-0.59$~pp), consistent with self-attenuation as $\delta \to 0$ (\S\ref{sec:boost}). On the high-imbalance text-dominant sentiment benchmarks (CMU-MOSI, CMU-MOSEI), where text alone is near-multimodally sufficient and prior balancing methods report only modest absolute gains~\citep{li2023agm,wei2025dgl}, PGGB+OGM-GE remains within seed standard deviation of the strongest baseline (MOSI $-0.08$~pp, MOSEI $-0.04$~pp). On BraTS~2021 (DeepLab v3+/ResNet-101, Dice loss), PGGB+OGM-GE yields $86.49\%$ Dice ($+0.72$~pp), extending the mechanism to dense prediction. The composability claim further extends on AVE to two of four non-OGM-GE methods tested (Appendix~\ref{app:composability}). A comparison against OPM~\citep{wei2024opm}, whose single-layer-fusion dependency requires a separate protocol, is in Appendix~\ref{app:opm_comparison}.
\vspace{-1.2em}
...
% [t] rather than [H]: under the ICLR layout a pinned figure here leaves page 8
% underfull and pushes the conclusion past the 9-page limit.
\begin{figure}[t]
\centering
...
We propose PGGB, a method for balanced multimodal learning that amplifies the contribution of weaker modalities while preserving stronger ones, composing naturally with existing throttling techniques. PGGB alone is the best-performing method on all four low-imbalance benchmarks. On high-imbalance CREMA-D, composition with OGM-GE yields $+2.31$~pp over the strongest baseline, and on the text-dominant sentiment benchmarks (CMU-MOSI, CMU-MOSEI) the same composition remains within seed standard deviation of the strongest baseline. On medical segmentation (BraTS~2021), it matches the strongest available baseline ($+0.28$~pp, within seed variance). \emph{Limitations.} Per-setting tuning is required, probes add small compute overhead, and composition with throttling can regress when the dominance gap is small (worst case AVE-from-scratch $-4.56$~pp, \S\ref{sec:operating_conditions}), a regime our theory does not cover. \emph{Future work.} Probe-driven adaptive composition that engages throttling only when the dominance gap warrants, and extension beyond classification and segmentation. No new data is introduced, and \S\ref{sec:operating_conditions} should precede deployment.


%=============================================================================
% ICLR 2027 END-OF-PAPER STATEMENTS (placed before the references,
% do not count toward the page limit, each at most 1 page)
%=============================================================================
\subsection*{AI use statement}
% REQUIRED by ICLR 2027. See the ICLR 2027 AI Policy for Authors.
\textcolor{red}{TODO (required): state the tasks for which generative AI tools were and were not used, how AI-assisted work was reviewed, and that the authors take responsibility for the final content.}

\subsection*{Ethics statement}
% RECOMMENDED. Candidate source material: checklist.tex (Safeguards and
% "New assets"/human-subjects justifications, public datasets, BraTS 2021 de-identification).
\textcolor{red}{TODO (recommended): ethics statement.}

\subsection*{Reproducibility statement}
% RECOMMENDED. Should point to the parts of the paper/supplement that support
% reproduction (Section 4.1, Appendix A proofs, Appendix B datasets, code in the
% supplementary material). Candidate source material: checklist.tex code-release justification.
\textcolor{red}{TODO (recommended): reproducibility statement.}
...
\subsection{Training algorithm pseudocode}
\label{app:algorithm}

Algorithm~\ref{alg:pgb} below specifies the full training pass referenced in Section~\ref{sec:boost}, making explicit how probe evaluation, weakness scoring, scale smoothing, and gradient boosting interleave with the forward and backward passes. The optional OGM-GE throttle line corresponds to the composition described in Section~\ref{sec:compose}.

\begin{algorithm}[H]
\caption{Probe-Guided Gradient Boosting (full training pass over $\mathcal{D}$). Notation: $P_m$ per-batch probe accuracy with smoothed estimate $\bar{P}_m$; $\phi$ fusion-head parameters; $\sg[\cdot]$ stop-gradient operator (\S\ref{sec:probes}). Auxiliary-loss weight $\gamma$ and optional OGM-GE throttle $\omega_m^{\text{ogm}} \leq 1$ are introduced in \S\ref{sec:compose}.}
\label{alg:pgb}
\begin{algorithmic}[1]
\REQUIRE Training data $\mathcal{D}$, encoders $\{\encoder_m\}_{m=1}^M$, fusion head $g$, unimodal heads $\{g_m\}$, probes $\{\probe_m\}$
...
\FOR{each mini-batch $(\mathbf{x}, y) \in \mathcal{D}$}
    ...
    \STATE \textbf{Loss:} $\loss \leftarrow \ell(g([\mathbf{z}_1;\ldots;\mathbf{z}_M]), y) + \gamma \sum_m \ell_{\text{aux}}(g_m(\mathbf{z}_m), y)$
...
\noindent\textbf{Licensing.} CREMA-D (CC-BY 4.0), AVE, Kinetics-Sounds, CMU-MOSI/MOSEI, Twitter15, Sarcasm (MMSD-v1), Food101 (UPMC research terms), and BraTS 2021 (RSNA-ASNR-MICCAI public release) are used under their published research terms.
...
\subsection{Per-dataset utilization gap}
\label{app:delta}

For transparency, we report the per-dataset utilization gap $\delta$ (Eq.~\eqref{eq:utilization_gap}) measured at the end of baseline training (final-epoch EMA-smoothed probe accuracy gap, averaged over 5 seeds). Probes follow the protocol of Section~\ref{sec:probes}: linear classifiers on stop-gradient encoder features, trained on a held-out within-batch sample. These values quantify modality-dominance dynamics during baseline training and define the high-imbalance ($\delta > 0.15$) and low-imbalance ($\delta \leq 0.15$) categorization used in Section~\ref{sec:imbalance_cat}. The high-imbalance group is heterogeneous in source: CREMA-D imbalance is audio-dominance in audio-visual emotion recognition, whereas CMU-MOSI and CMU-MOSEI imbalance reflects text-sufficiency, where text-only baselines are reported within roughly $1$~pp of multimodal accuracy~\citep{li2023agm,wei2025dgl}. BraTS~2021 (4 MRI sequences, binary tumor-presence probes per sequence) is reported separately as a dense-prediction benchmark.

\begin{table}[H]
\centering
\caption{Per-dataset utilization gap $\delta$ at the end of baseline training (mean $\pm$ std over 5 seeds), measured as the EMA-smoothed probe accuracy gap (Eq.~\eqref{eq:utilization_gap}).}
\label{tab:delta_per_dataset}
\vspace{0.3em}
\begin{tabular}{lc}
\toprule
\textbf{Dataset} & $\boldsymbol{\delta}$ \textbf{(mean $\pm$ std)} \\
\midrule
CREMA-D       & $0.268 \pm 0.029$ \\
CMU-MOSI      & $0.222 \pm 0.047$ \\
CMU-MOSEI     & $0.178 \pm 0.086$ \\
Twitter15     & $0.150 \pm 0.096$ \\
Sarcasm       & $0.076 \pm 0.008$ \\
Kinetics-Sounds & $0.038 \pm 0.013$ \\
AVE           & $0.017 \pm 0.009$ \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Cross-class comparison with MSLR}
\label{app:mslr}

We additionally compare PGGB to MSLR~\citep{yao2022mslr}, a per-modality learning-rate (PMLR) baseline. As discussed in Section~\ref{sec:related}, MSLR operates on the optimizer step rather than the gradient and forms an adjacent intervention class that is not directly composable with gradient-modulation methods. Because MSLR's canonical Keep-variant requires per-modality unimodal pre-calibration that lies outside our matched-pipeline budget, we run a fixed-ratio Keep variant: audio:1.0/visual:2.0 on CREMA-D (selected from a 3-config sweep over audio:0.25/0.5/1.0 with matching visual scaling), audio:0.5/visual:1.0 on AVE and Kinetics-Sounds, and text:0.5 with other modalities at 1.0 on CMU-MOSI, CMU-MOSEI, Twitter15, and Sarcasm. Five seeds per dataset on the matched ResNet-18 / two-layer MLP pipeline.

Table~\ref{tab:mslr_comparison} reports the comparison. PGGB+OGM-GE exceeds MSLR by $+7.66$~pp on the high-imbalance CREMA-D benchmark and exceeds MSLR on the high-imbalance text-dominant sentiment benchmarks (MOSI $+0.76$~pp, MOSEI $+1.44$~pp), supporting the composability story. PGGB alone matches or exceeds MSLR on three of four low-imbalance comparisons (AVE, KS, Sarcasm) and is within seed variance on Twitter15 ($-0.40$~pp, sub-$\sigma$). MSLR's heuristic throttle-audio ratio underperforms baseline on AVE ($-1.82$~pp), illustrating the sensitivity of fixed-ratio MSLR to per-dataset modality-dominance patterns that PGGB's probe-driven adaptation sidesteps.

\begin{table}[H]
\centering
\caption{Cross-class comparison with MSLR~\citep{yao2022mslr} (5 seeds, mean $\pm$ std). MSLR runs the fixed-ratio Keep variant, ratios per dataset listed above. PGGB family results reproduced from Table~\ref{tab:main_results} for reference. \textbf{Bold} indicates the best per-row mean.}
\label{tab:mslr_comparison}
\vspace{0.3em}
\footnotesize
\begin{tabular}{lcccc}
\toprule
\textbf{Dataset} & MSLR & PGGB ($\alpha{=}0.5$) & PGGB+OGM-GE ($\alpha{=}0.75$) & Best \\
\midrule
CREMA-D    & 63.79{\scriptsize$\pm$0.47} & 62.72{\scriptsize$\pm$1.65} & \textbf{71.45}{\scriptsize$\pm$1.71} & \textbf{PGGB+OGM-GE} \\
AVE        & 84.72{\scriptsize$\pm$0.59} & \textbf{87.41}{\scriptsize$\pm$0.26} & 87.23{\scriptsize$\pm$0.58} & \textbf{PGGB} \\
Kinetics-Sounds & 78.56{\scriptsize$\pm$0.38} & \textbf{79.17}{\scriptsize$\pm$0.97} & 77.33{\scriptsize$\pm$0.64} & \textbf{PGGB} \\
CMU-MOSI   & 71.84{\scriptsize$\pm$0.76} & 71.89{\scriptsize$\pm$0.82} & \textbf{72.60}{\scriptsize$\pm$0.94} & \textbf{PGGB+OGM-GE} \\
CMU-MOSEI  & 70.99{\scriptsize$\pm$1.03} & 69.80{\scriptsize$\pm$0.80} & \textbf{72.43}{\scriptsize$\pm$0.65} & \textbf{PGGB+OGM-GE} \\
Twitter15  & \textbf{67.25}{\scriptsize$\pm$0.66} & 66.85{\scriptsize$\pm$0.50} & 66.59{\scriptsize$\pm$0.51} & \textbf{MSLR} (sub-$\sigma$) \\
Sarcasm    & 82.37{\scriptsize$\pm$0.32} & \textbf{82.44}{\scriptsize$\pm$0.35} & 81.76{\scriptsize$\pm$0.08} & \textbf{PGGB} \\
\bottomrule
\end{tabular}
\end{table}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% NeurIPS Paper Checklist removed: ICLR has no checklist. checklist.tex is
% kept on disk as source material for the ethics/reproducibility statements.
