...
\begin{itemize}
    ...
    \item We validate \emph{PGGB} on eight benchmarks spanning audio-visual, sentiment, text-image, and medical-segmentation tasks. \emph{PGGB} composed with throttling outperforms state-of-the-art baselines on high-imbalance audio-visual (CREMA-D) and matches or exceeds the strongest available baseline on medical segmentation (BraTS~2021), and \emph{PGGB} used alone remains competitive with the strongest baselines on the remaining low-imbalance datasets, with isolated regressions noted in Section~\ref{sec:conclusion}.
\end{itemize}
