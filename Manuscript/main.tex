...
\begin{itemize}
    ...
    \item We validate \emph{PGGB} on eight benchmarks across classification, regression, and segmentation. The positive effect is concentrated on high-imbalance audio-visual (CREMA-D) and medical segmentation (BraTS~2021), with competitive performance and isolated regressions on low-imbalance benchmarks (Section~\ref{sec:conclusion}).
\end{itemize}
...
We propose PGGB, a method for balanced multimodal learning that amplifies weaker modalities while preserving stronger ones, composing naturally with existing throttling techniques. On the high-imbalance CREMA-D benchmark, composition with OGM-GE yields $+2.31$~pp over the strongest baseline. Outside the high-imbalance regime the method is conditionally effective, with regressions of $-0.6$ to $-4.6$~pp on certain low-imbalance benchmarks (\S\ref{sec:operating_conditions}). \emph{Limitations.} Per-setting hyperparameter tuning is required, and probes introduce a small computational overhead. \emph{Future work.} Automatic hyperparameter adaptation, broader benchmark coverage, and extension beyond classification and segmentation. No new data is introduced, and \S\ref{sec:operating_conditions} should precede deployment.
\vspace{-1.8em}
...
On CREMA-D, PGGB+OGM-GE exceeds OPM+OGM by $+1.93$~pp under OPM's native single-layer-fusion pipeline ($71.77\%$ vs.\ $69.84\%$).
...
\begin{table}[H]
\centering
\caption{Composability with existing balancing methods on CREMA-D (3-frame, mean $\pm$ std over 5 seeds). All rows use identical PGGB hyperparameters. Positive $\Delta$ indicates composition does not hurt the base method's mean accuracy.}
