...
\end{equation}
%
Each $\mathbf{z}_m$ is a $d$-dimensional real vector with $d$ common across modalities. The features are concatenated and passed through a fusion classifier $g$ parameterized by $\phi$, yielding $\hat{y} = g([\mathbf{z}_1;\ldots;\mathbf{z}_M];\,\phi)$. Training minimizes the cross-entropy loss:
%
\begin{equation}
\label{eq:joint_loss}
