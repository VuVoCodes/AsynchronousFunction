...
\emph{Reproduction protocol.} All baselines are re-run under a matched pipeline (shared encoders, 3-frame CREMA-D, identical augmentation and optimizer) for apples-to-apples comparison, differing from original publications. OGM-GE ($69.14\%$) is $3$--$6$~pp below the $72$--$75\%$ in follow-up work that used different frame sampling, AGM ($57.42\%$) uses our ResNet-18 rather than its original backbone, MILES is ported without its specialized architecture, and CGGM from its Transformer setting (noted above). This trades per-method tuning for cross-method comparability.
...
\paragraph{Isolating the monitor from the boost.} To disentangle monitoring from actuation, we decompose CREMA-D 3-frame (Table~\ref{tab:main_results} protocol, 5 seeds) into: (i) OGM-GE alone ($69.14 \pm 1.13\%$), (ii) Monitor+OGM-GE with $\alpha{=}0$ (probes active, scale held at unity, $69.14 \pm 1.18\%$), and (iii) Boost+OGM-GE with $\alpha{=}0.75$ ($71.45 \pm 1.71\%$). The $\alpha{=}0$ condition matches OGM-GE alone in mean and spread, so the monitor is a passive diagnostic and the full $+2.31$~pp gain is attributable to boost actuation, not to probe-feature side-effects or regularization from probe training.
...
Aggregate accuracy conflates encoder capacity with fusion, so we report post-hoc linear-probe accuracy on frozen encoders (Appendix~\ref{app:posthoc_probe_table}). Boost+OGM-GE attains the smallest utilization gap ($2.31$~pp, Figure~\ref{fig:probe_accuracy}b), a $5.4\times$ reduction from baseline's $12.53$~pp, with asymmetric narrowing: the weak probe rises $+9.41$~pp while the strong probe falls only $-0.81$~pp. AGM, MILES, and G-Blend instead degrade the weak probe (failure modes invisible in aggregate accuracy). The method extends to $M$ modalities via pairwise weakness scores (Eq.~\ref{eq:weakness}).
...
\vspace{-0.6em}
Probe-guided gradient boosting converts decoupled linear-probe estimates into a bounded EMA-smoothed scaling on the weaker encoder, with proven bounded scaling, self-attenuation, and descent, yielding $+2.31$~pp over OGM-GE on CREMA-D.
\vspace{-0.6em}
\paragraph{Limitations.}
The method is conditionally effective (\S\ref{sec:operating_conditions}), with small regressions on low-imbalance data ($-0.62$/$-0.64$/$-1.72$~pp on MOSEI/Sarcasm/KS). Probe estimates are subsample-noisy and EMA smoothing ($\mu{=}0.3$) only partially mitigates this. The convergence bound inflates gradient variance by $s_{\max}^{2}$ (Appendix~\ref{app:proofs}), $\alpha$ is tuned per setting, and probes add $\sim$$1\%$ overhead.
\vspace{-0.6em}
\paragraph{Broader impacts.}
The method alters training dynamics and introduces no new data or generative capabilities. It can harm performance outside its operating regime ($-4.56$~pp on AVE from scratch), so the \S\ref{sec:operating_conditions} diagnostic should precede deployment.
