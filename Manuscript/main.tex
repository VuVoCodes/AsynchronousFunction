...
Every $K$ training iteration, we partition the (shuffled) mini-batch into two disjoint halves, training probes on the first half and evaluating on the second. This yields a \emph{conditionally unbiased} estimate $P_m$ of the current probe's training-distribution accuracy (conditioned on $\psi_m^{(t)}$), the evaluation half is held out from probe training and drawn i.i.d.\ from the training distribution. To convert this per-evaluation sample into a stable estimator, we accumulate an exponential moving average (EMA):
%
\begin{equation}
\label{eq:ema_probe}
...
\end{equation}
%
with $\epsilon$ as a small positive constant serving as a numerical guard against division by zero.
...
\end{equation}
%
where $\gamma$ is a scalar weight on the per-modality auxiliary cross-entropy terms (set in Section~\ref{sec:experiments}).
