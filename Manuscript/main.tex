...
\end{equation}
%
Each $\mathbf{z}_m$ is a $d$-dimensional real value vector with $d$ common across modalities. The features are concatenated and passed through a fusion head $g$ parameterized by $\phi$, yielding the prediction $\hat{y} = g([\mathbf{z}_1;\ldots;\mathbf{z}_M];\,\phi)$. With ground-truth target $y$, training minimizes a task-appropriate fusion loss:
%
\begin{equation}
\label{eq:joint_loss}
