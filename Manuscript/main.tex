...
\end{equation}
%
with $\epsilon = 10^{-8}$ as a numerical guard against division by zero.\footnote{Implementation uses a hard branch returning $s_m = 1$ when the gap falls below $\epsilon$, coinciding with Eq.~\ref{eq:weakness} at $\Delta = 0$.}
...
\vspace{-1.2em}
\caption{Per-modality diagnostics on CREMA-D (5 seeds). \textbf{(a)} Post-hoc linear-probe accuracy $P_m^{\text{post}}$ (Appendix~\ref{app:posthoc_probe_table}), dotted line marks chance ($1/6$). AGM and MILES collapse the weak probe despite competitive aggregate accuracy. \textbf{(b)} Utilization gap $|P_a^{\text{post}} - P_v^{\text{post}}|$. Boost+OGM-GE attains the tightest gap ($2.31$~pp, $5.4\times$ reduction). Error bars: $\pm 1$ std.}
...
\vspace{-1em}
We introduced probe-guided gradient boosting: a bounded, EMA-smoothed multiplicative scale on the weaker encoder's gradient driven by stop-gradient probes, composable with throttling methods such as OGM-GE. On CREMA-D it yields $+2.31$~pp under proven safety properties, but is conditionally effective (\S\ref{sec:operating_conditions}: $-0.6$ to $-4.6$~pp on MOSEI/Sarcasm/KS/AVE). \emph{Limitations:} subsample-noisy probe estimates, per-setting $\alpha$, $s_{\max}^{2}$ variance inflation, and $\sim$$1\%$ probe overhead. No new data or generative capability is introduced, and \S\ref{sec:operating_conditions} should precede deployment.
